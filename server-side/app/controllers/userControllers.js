    ...
    try {
        ...
        if (req.files && req.files.image) {
            ...
            const fetchImageQuery = `
                ...
                WHERE id = $1
            `;
            const result = await pool.query(fetchImageQuery, [userId]);
            if (result.rows.length > 0) {
                imagePaths.push(result.rows[0].image);
            }
        }
        ...
        const updatedUser = await pool.query(updateUserQuery, [
            ...
            req.body.businessname ? req.body.businessname : user.businessname,
            imagePaths[0], // Use the first image path
            req.body.location ? req.body.location : user.location,
            req.body.phone ? req.body.phone : user.phone,
            req.body.bio ? req.body.bio : user.bio,
            userId,
        ]);

        res.status(200).json({ success: true, message: 'Updated successfully', user: updatedUser.rows[0] });
    } catch (error) {
        console.error(error);
...
exports.forgetLink = catchAsyncFunction(async(req, res) => {
    ...
    if (user.rows.length == 0) {
        ...
        var link = `${process.env.frontendUrl}/forgotpassword/${hashedlink}`;
        console.log(link);
        var mailOptions = {
            from: 'testing@cognuitive.com',
            to: req.body.email,
...
exports.userByToken = catchAsyncFunction(async(req, res) => {
    const userToken = await pool.query(
        "select * from user_forget_password_token where token=$1", [req.body.token]
    )
    res.json({
        success:true,
        data:userToken.rows[0]
    })
})
